...
commentRouter.post('/', (req: Request, res: Response, next: NextFunction) => {
    ...
    if (checker.notNull(authorId, author, postId, content)) {
        commentDatabase.createComment(authorId, postId, author, 0, 0, now(), target, content, ContentStatus.GOOD);
...
commentRouter.patch('/', (req: Request, res: Response, next: NextFunction) => {
    let content: string = req.body.content;
    ...
    if (checker.notNull(content, uid)) {
        commentDatabase.updateComment(uid, content);
        res.status(200).send(respRest(200, 0));
    } else {
        res.status(400).send(respRest(400, 1));
    }
...
commentRouter.delete('/', (req: Request, res: Response, next: NextFunction) => {
    let uid: string = req.body.uid;

    let checker = new QueryChecker();

    if (checker.notNull(uid)) {
        commentDatabase.deleteCommentByUid(uid);
        res.status(200).send(respRest(200, 0));
    } else {
        res.status(400).send(respRest(400, 1));
    }
